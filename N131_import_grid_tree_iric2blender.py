    ...
    def execute(self, context):
        ...
        def grow_tree(ob1,faces_tree,addon_dirpath):
            ...
            if vm==1:
                ...
                for tree_name in gazou_trees:
                    bpy.ops.import_scene.fbx(filepath=f'{addon_dirpath}/tree_data/gazou_tree/{tree_name}.fbx')
                # 全ての植生を格納するリストを作成
                trees_all=gazou_trees
                trees_all.extend(gazou_trees)
                #trees_all.extend(new_trees)
                self.report({'INFO'}, f"trees_all:{trees_all}")
            ...
            for face in mesh.polygons:
                ...
                grow_tree=[]
                if zratio == 0:
                    if (faces_tree[j][4] >= 0.01) & (faces_tree[j][7] >= 0):
